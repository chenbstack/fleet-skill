...
instance**,不是 state.yaml 里写的 hosts。两边不一致时 preflight 会报
...
>    上传到 `/apps/{app}/site:upload`。站点不存在时 panel 用 state.yaml 的
...
cert / dns / acme / drift(这些需要 state.yaml 的 tls 意图,服务端拿不到),
